...
def _getWorldSharedTextureDirs(texture_dir):
    texture_path = Path(texture_dir).resolve()
    data_dir = None
    source_group = None
    for ancestor in (texture_path, *texture_path.parents):
        if ancestor.name.lower().startswith('data'):
            data_dir = ancestor
            try:
                relative_parts = texture_path.relative_to(ancestor).parts
                source_group = relative_parts[0]
            except (ValueError, IndexError):
                return ()
            break

    if data_dir is None or not source_group.lower().startswith('wd') or source_group.lower() == 'wda':
        return ()

    cpk_root = data_dir.parent
    try:
        extracted_index = tuple(part.lower() for part in relative_parts).index('nier2blender_extracted')
        archive_stem = Path(relative_parts[extracted_index + 1]).stem
    except (ValueError, IndexError):
        return ()

    family_data_dirs = [data_dir]
    local_dat_path = data_dir / source_group / (archive_stem + '.dat')
    if not local_dat_path.is_file():
        for sibling_data_dir in sorted(cpk_root.iterdir(), key=lambda path: path.name.lower()):
            if not sibling_data_dir.is_dir() or not sibling_data_dir.name.lower().startswith('data'):
                continue
            companion_path = sibling_data_dir / source_group / (archive_stem + '.dat')
            if companion_path.is_file():
                family_data_dirs.insert(0, sibling_data_dir)
                break

    shared_dat_stems = set()
    shared_dtt_paths = {}
    for family_data_dir in family_data_dirs:
        shared_dir = family_data_dir / 'wda'
        if not shared_dir.is_dir():
            continue
        shared_dat_stems.update(path.stem.lower() for path in shared_dir.glob('*.dat'))
        for dtt_path in shared_dir.glob('*.dtt'):
            shared_dtt_paths.setdefault(dtt_path.stem.lower(), dtt_path)

    shared_texture_dirs = []
    for stem in sorted(shared_dat_stems & shared_dtt_paths.keys()):
        dtt_path = shared_dtt_paths[stem]
        archive_dir = dtt_path.parent / 'nier2blender_extracted' / (dtt_path.stem + '.dtt')
        if archive_dir.is_dir():
            standard_texture_dir = archive_dir / 'textures'
            if standard_texture_dir.is_dir():
                shared_texture_dirs.append(str(standard_texture_dir))
            for extracted_dir in sorted(archive_dir.glob('*.wtp_extracted'), key=lambda path: path.name.lower()):
                if extracted_dir.is_dir():
                    shared_texture_dirs.append(str(extracted_dir))
    return tuple(shared_texture_dirs)
...
def getTexture(texture_dir, texture_name):
    ...
    # World DTTs under wd* share textures stored in the sibling wda archives.
    # Search those extracted archives automatically before user preference fallbacks.
    for shared_texture_dir in _getWorldSharedTextureDirs(texture_dir):
        texture_search = os.path.join(shared_texture_dir, ''.join(either(char) for char in texture_name) + ".*")
        texture_files = glob.glob(texture_search, recursive=True)
        if len(texture_files) > 0:
            return texture_files[0]
