...
Terrain::Terrain(const Shared<Terrain>& rhs)
	: GameObject{ rhs } {}
...
{
	GAME_INSTANCE->Add_RenderGroup(RENDERGROUP::NONBLEND, enable_shared_from_this<Terrain>::shared_from_this());
}
...
{
	if (m_Transform->Bind_ShaderResource(m_Shader, "g_WorldMatrix"))
		return E_FAIL;
	if (GAME_INSTANCE->Bind_TransformMatrix(m_Shader, "g_ViewMatrix", D3DTS::VIEW))
		return E_FAIL;
	if (GAME_INSTANCE->Bind_TransformMatrix(m_Shader, "g_ViewMatrix", D3DTS::PROJ))
		return E_FAIL;
	...
	//const LIGHT_DESC* lightDesc = GAME->Get_LightDesc(0);
...
Shared<Terrain> Terrain::Create(const ComPtr<ID3D11Device>& device, const ComPtr<ID3D11DeviceContext>& context)
{
...
{
	auto terrain = make_shared<Terrain>(enable_shared_from_this<Terrain>::shared_from_this());
