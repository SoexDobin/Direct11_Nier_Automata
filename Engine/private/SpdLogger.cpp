...
void SpdLogger::Initialize() {
  try {
    ...
    g_Logger = std::make_shared<spdlog::logger>(
        "EngineLogger", sink_list.begin(), sink_list.end());
...
void SpdLogger::SetLogCallback(LogCallback callback) {
...
void SpdLogger::Critical(const std::string &message) {
  ...
  if (g_LogCallback)
    g_LogCallback(LogLevel::Critical, message);
}

/* ===== wstring → string 변환 후 호출 ===== */
...
void SpdLogger::Initialize() {}
void SpdLogger::Shutdown() {}
void SpdLogger::SetLogCallback(LogCallback) {}
