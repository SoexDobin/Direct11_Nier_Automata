...
HRESULT GraphicDevice::Ready_SwapChain(HWND hWnd, WINMODE isWindowed,
    ...
    if (FAILED(m_Device->QueryInterface(__uuidof(IDXGIDevice), reinterpret_cast<void**>(device.GetAddressOf()))))
        return E_FAIL;
    ...
    if (FAILED(adapter->GetParent(__uuidof(IDXGIFactory2), reinterpret_cast<void**>(factory2.GetAddressOf()))))
        return E_FAIL;
  ...
  return factory2->CreateSwapChainForHwnd(
      m_Device.Get(),
      hWnd,
      &swapChain,
      &fullScreenSwapChain,
