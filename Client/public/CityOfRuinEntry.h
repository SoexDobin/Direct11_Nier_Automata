...
class CLIENT_DLL CityOfRuinEntry final : public WorldObject
{
	RTTR_ENABLE(WorldObject)
public:
	explicit CityOfRuinEntry() = default;
	explicit CityOfRuinEntry(const ComPtr<ID3D11Device>& device, const ComPtr<ID3D11DeviceContext>& context);
	explicit CityOfRuinEntry(const CityOfRuinEntry& rhs);
	~CityOfRuinEntry() override = default;
