...
void UIObject::Priority_Update(Float timeDelta) { GameObject::Priority_Update(timeDelta); }
void UIObject::Update(Float timeDelta) { GameObject::Update(timeDelta); }
void UIObject::Late_Update(Float timeDelta) { GameObject::Late_Update(timeDelta); }
void UIObject::Fixed_Update(Float fixedDelta) { GameObject::Fixed_Update(fixedDelta); }
HRESULT UIObject::Render() { return GameObject::Render(); }

void UIObject::Update_Transform() const
{
	m_Transform->Set_Scale(m_SizeX, m_SizeX, 1.f);
	m_Transform->Set_Position(Vector3{ m_X - m_ViewportWidth * 0.5f, m_Y - m_ViewportHeight * 0.5f, 0.f});
}

HRESULT UIObject::Bind_ShaderResource(const Shared<Shader>& shader, const Char* constantName, D3DTS transformState) const
{
