...
public:
    explicit LoadingFadeIn();
    explicit LoadingFadeIn(const ComPtr<ID3D11Device>& device, const ComPtr<ID3D11DeviceContext>& context);
    explicit LoadingFadeIn(const LoadingFadeIn& rhs);
    ~LoadingFadeIn() override = default;
...
public:
    static Shared<LoadingFadeIn> Create(const ComPtr<ID3D11Device>& device, const ComPtr<ID3D11DeviceContext>& context);
