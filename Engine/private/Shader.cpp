...
HRESULT Shader::Initialize_Prototype(const tChar* shaderFilePath, const D3D11_INPUT_ELEMENT_DESC* elements, uint32 numElements)
{
	...
	if (FAILED(D3DX11CompileEffectFromFile(shaderFilePath,
										   nullptr,
	...
	if (ComPtr<ID3DX11EffectTechnique> technique = m_Effect->GetTechniqueByIndex(0))
	{
		...
		for (uint32 i = 0; i < m_NumPasses; ++i)
		{
			...
			if (nullptr == pass) return E_FAIL;

			D3DX11_PASS_DESC passDesc = {};
			pass->GetDesc(&passDesc);

			if (FAILED(m_Device->CreateInputLayout(elements,
												   numElements,
...
HRESULT Shader::Initialize(const Shared<void>& arg)
{
...
Shared<Component> Shader::Clone(const Shared<void>& arg)
{
