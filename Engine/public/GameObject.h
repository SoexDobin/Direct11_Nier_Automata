...
public:
	...
	{
		uint32 level = levIndex == UINT_MAX ? Game::GetInstance()->Get_CurrentLevelIndex() : levIndex;
		if (Shared<const T> prototype = Game::GetInstance()->Find_Prototype<T>(PROTOTYPE::COMPONENT, level))
		{
			if (prototype->Get_ComponentType() == COMPONENT_TYPE::SCRIPT && m_Scripts.contains(prototype->Get_ObjectID()))
			{
...
protected:
	...
	Shared<T> Add_Component(void* arg = nullptr, uint32 levIndex = UINT_MAX)
	{
		...
		if (Shared<T> instance = Game::GetInstance()->Instantiate<T>(arg)) {
			if (instance->Get_ComponentType() == COMPONENT_TYPE::SCRIPT && !m_Scripts.contains(instance->Get_ObjectID()))
			{
