...
LoadingFadeIn::LoadingFadeIn() : UIObject() {}
LoadingFadeIn::LoadingFadeIn(const ComPtr<ID3D11Device>& device, const ComPtr<ID3D11DeviceContext>& context)
    : UIObject(device, context) {}
LoadingFadeIn::LoadingFadeIn(const LoadingFadeIn& rhs) : UIObject(rhs) {}

HRESULT LoadingFadeIn::Initialize_Prototype() {
    return UIObject::Initialize_Prototype();
}

HRESULT LoadingFadeIn::Initialize(void* arg)
{
    if (nullptr == arg)
    {
        m_FadeSpeed = 0.25f;
    ...
    {
        FADE_IN_DESC& desc = *static_cast<FADE_IN_DESC*>(arg);
    ...
    // FadeIn (Fade to Black) is usually inactive at start until transition
    Set_Active(false);

    return S_OK;
}

void LoadingFadeIn::On_Destroy() { UIObject::On_Destroy(); }

void LoadingFadeIn::Update(Float timeDelta) 
{
    Update_UITransform();
...
void LoadingFadeIn::Submit_RenderGroup()
{
    GAME_INSTANCE->Add_RenderGroup(RENDERGROUP::UI, shared_from_this());
}

void LoadingFadeIn::Reset_Fade(Float fadeSpeed)
{
    m_IsFadeEnd = false;
...
Shared<LoadingFadeIn> LoadingFadeIn::Create(const ComPtr<ID3D11Device>& device, const ComPtr<ID3D11DeviceContext>& context) {
    auto logo = make_shared<LoadingFadeIn>(device, context);

    if (FAILED(logo->Initialize_Prototype())) {
        LOG_ERROR(L"Failed To Create LoadingFadeIn");
        return nullptr;
    }
...
Shared<GameObject> LoadingFadeIn::Clone(void* arg) {
    auto logo = make_shared<LoadingFadeIn>(*this);

    if (FAILED(logo->Initialize(arg))) {
        LOG_ERROR(L"Failed To Create LoadingFadeIn Clone");
        return nullptr;
    }
