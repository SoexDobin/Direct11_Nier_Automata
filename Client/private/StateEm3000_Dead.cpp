...
{
	// 사망 애니메이션 시작 (애니메이션 인덱스는 프로젝트 정의에 따름)
	// m_Body.lock()->Change_Animation(EM3000_STATE::DEAD); // 필요시 추가
...
void StateEm3000_Dead::Update(Float timeDelta)
{
	...
	if (m_ExplosionTimer >= m_ExplosionInterval)
	{
		...
		// 랜덤한 폭발 사운드 (필요시)
		GAME_INSTANCE->PlaySoundFXOnce(L"Explosion_Sound", SOUNDCHANNEL::CHANNEL_27, 0.2f);
	}
